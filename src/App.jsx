      ...
      <main className="p-4 max-w-2xl mx-auto">
        ...
        {activeTab === 'marketing' && (
          <div className="space-y-6 pb-6">
                ...
                {SHOOTING_LIST.map((task) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-3 rounded-lg flex items-center gap-3 cursor-pointer transition-all ${isDone ? 'opacity-50 bg-slate-50' : 'hover:bg-slate-50'}`}
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between">
                          ...
                          <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-medium">{task.time}</span>
                        </div>
            ...
            {/* List 2: Edit Schedule (Tabbed Interface) */}
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-slate-50 p-4 border-b border-slate-100 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <Scissors className="w-5 h-5 text-purple-600" />
                  <h2 className="font-bold text-slate-800">2. Editing Schedule</h2>
                </div>
              </div>
              ...
              <div className="flex border-b border-slate-100 overflow-x-auto">
                {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map(day => (
                  <button 
                    key={day}
                    onClick={() => setMarketingDay(day)}
                    className={`flex-1 py-3 text-xs font-bold transition-colors ${marketingDay === day ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    {day}
                  </button>
                ))}
              ...
              <div className="p-4">
                <div className="mb-4">
                  <h3 className="text-lg font-bold text-slate-800">{EDITING_STRATEGY[marketingDay].title}</h3>
                  <p className="text-sm text-slate-500 mt-1">{EDITING_STRATEGY[marketingDay].subject}</p>
                </div>

                <div className="space-y-3">
                  {EDITING_STRATEGY[marketingDay].platforms.map((platform, idx) => {
                    return (
                      <div key={idx} className="bg-slate-50 rounded-lg p-3 border border-slate-100">
                        <div className="flex items-center gap-2 mb-2">
                          {platform.name === 'TikTok' && <span className="bg-black text-white text-[10px] font-bold px-2 py-0.5 rounded">TikTok</span>}
                          {platform.name === 'Instagram' && <span className="bg-pink-600 text-white text-[10px] font-bold px-2 py-0.5 rounded">IG</span>}
                          {platform.name === 'Facebook' && <span className="bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded">FB</span>}
                        </div>
                        <div className="grid grid-cols-1 gap-2 text-xs">
                          <div className="flex gap-2"><span className="font-bold w-10 shrink-0 text-slate-400">Style:</span> <span className="text-slate-700">{platform.style}</span></div>
                          <div className="flex gap-2"><span className="font-bold w-10 shrink-0 text-slate-400">Audio:</span> <span className="text-slate-700">{platform.audio}</span></div>
                          <div className="flex gap-2"><span className="font-bold w-10 shrink-0 text-slate-400">Tags:</span> <span className="text-blue-600 font-medium">{platform.text}</span></div>
                        </div>
                      </div>
                    )
                  })}
                ...
                {/* Completion Toggle for the Day */}
                <button 
                  onClick={() => handleMarketingToggle(`edit_${marketingDay}`)}
                  className={`mt-4 w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${
                    firestoreData.marketing?.[`edit_${marketingDay}`] 
                      ? 'bg-green-100 text-green-700' 
                      : 'bg-slate-900 text-white hover:bg-slate-800'
                  }`}
                >
                  {firestoreData.marketing?.[`edit_${marketingDay}`] ? (
                    <> <CheckSquare className="w-5 h-5" /> Completed for {marketingDay} </>
                  ) : (
                    "Mark Day Complete"
                  )}
                </button>
